...
fn convert<const N: usize>(source_image: RawTga<'_>, png_format: png::ColorType, convert: impl FnMut(RawPixel) -> [u8; N]) -> JoinHandle<Vec<u8>> {
    ...
    let convert_future = tokio::task::spawn_blocking(move || {
        ...
        png_encoder.set_color(png_format);
...
fn rgb565_to_888(pixel: RawPixel) -> [u8; 3] {
...
fn rgb4444_to_8888(pixel: RawPixel) -> [u8; 4] {
...
fn rgb88_to_888(pixel: RawPixel) -> [u8; 3] {
    let u: u8 = ((pixel.color << 0) & 0xFF) as u8;
    let v: u8 = ((pixel.color >> 8) & 0xFF) as u8;
    [u, v, 0]
}